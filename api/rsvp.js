...
// Proxy para o seu Google Apps Script (GAS)
// Produção: só permite seu domínio vercel.app
...
function isAllowedOrigin(origin) {
  ...
  // Autoriza produção e previews da Vercel (ex.: https://<branch>-casamento-nu-black.vercel.app)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Se o GAS devolver JSON, repassa como JSON; senão repassa texto
    try {
    ...
    return res.status(502).json({
      error: "Falha ao contactar o Apps Script",
      detail: String(err),
    });
  }
}
